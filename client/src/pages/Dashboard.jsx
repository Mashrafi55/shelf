...
function Dashboard() {
  ...
  if (loading) return (
    <div style={{ backgroundColor: bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ fontFamily: 'Newsreader, serif', fontSize: '24px', color: muted, fontStyle: 'italic' }}>Loading your shelf...</p>
    </div>
  )
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        {currentlyReading?.book && (
          <div style={{
            ...
            <div style={{
              ...
              overflow: 'hidden', backgroundColor: border, flexShrink: 0,
            }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              }}>{currentlyReading.book.title}</h3>
              <p style={{
                fontFamily: 'Manrope, sans-serif', fontSize: '13px',
                color: muted, marginBottom: '12px',
              }}>Page {currentlyReading.lastPage}</p>
              <div style={{ height: '3px', backgroundColor: border, borderRadius: '999px', marginBottom: '12px' }}>
                <div style={{
          ...
          <div>
            {library.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px' }}>
                <p style={{ fontFamily: 'Newsreader, serif', fontSize: '24px', color: muted, fontStyle: 'italic', marginBottom: '16px' }}>
                  ...
                  <Link key={book._id} to={`/read/${book._id}`} style={{ textDecoration: 'none' }}>
                    <div style={{
                      ...
                      <div style={{ padding: '16px' }}>
                        <h3 style={{ fontFamily: 'Newsreader, serif', fontSize: '18px', color: text, marginBottom: '4px' }}>
                          {book.title}
                        </h3>
                        <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: '12px', color: muted, fontStyle: 'italic' }}>
                          {book.author}
                        </p>
                      </div>
                    </div>
        ...
        {tab === 'bookmarks' && (
          <div>
            {bookmarks.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px' }}>
                <p style={{ fontFamily: 'Newsreader, serif', fontSize: '24px', color: muted, fontStyle: 'italic' }}>
                  No bookmarks yet.
                </p>
              </div>
            ) : (
                ...
                {bookmarks.map(bookmark => (
                  <div key={bookmark._id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                      ...
                      <div>
                        <p style={{ fontFamily: 'Newsreader, serif', fontSize: '18px', color: text }}>
                          {bookmark.book?.title}
                        </p>
                        <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: '12px', color: muted }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                      <Link to={`/read/${bookmark.book?._id}`} style={{
                        ...
                        letterSpacing: '0.05em', textDecoration: 'none', fontWeight: '600',
                      }}>Go to page</Link>
                      <button onClick={() => deleteBookmark(bookmark._id)} style={{
                        background: 'none', border: 'none', cursor: 'pointer',
                        color: muted, fontFamily: 'Manrope, sans-serif', fontSize: '12px',
                      }}>Remove</button>
                    </div>
                  </div>
                ))}
